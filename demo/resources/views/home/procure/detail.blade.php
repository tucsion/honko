...
<script type="text/javascript">
    document.title='{{$detail -> name}}-合作企业';
</script>
...
<style>
	...
	<!-- banner -->
	<div id="banner" style="background: url('/updates/{{$banner -> img}}') center center no-repeat;"></div>
	<div id="list" class="container">
		<!-- 左边栏 -->
		<div id="left">
			<dl>
				<dt>企业采购</dt>
				@foreach($procure as $qy)
				<dd 	
				@if($detail-> cid  == $qy -> id)
				class="active"
				@endif>
					<i class="layui-icon transition">&#xe602;</i>
					<a class="transition" href="{{ url('/procure/index') }}/{{ $qy -> id }}">{{$qy -> cate}}</a>
				</dd>
				@endforeach
			</dl>
		</div>
